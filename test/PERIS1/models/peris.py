...
class PERIS(nn.Module):
    ...
    def compute_loss(self, batch_data):
        ...
        df = pd.DataFrame(zip(users, items, times), columns = ['user_id', 'user_item', 'times'])

        return df, loss, loss_IS4debug # loss_IS is a redudant loss so it's okay
    
    def compute_warmup_loss(self, batch_data):        
        users, items, times, user_timebins, history, _, all_neg_items, _, _, _, _, _ = batch_data
        df = pd.DataFrame(zip(users, items, times), columns = ['user_id', 'user_item', 'times'])
        return df, self.get_pref_loss(users, items, all_neg_items) 
