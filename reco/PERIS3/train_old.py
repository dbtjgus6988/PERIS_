...
class Instructor:
            ...
            for i, batch_data in enumerate(self.trn_loader):
                ...
                if opt.model_name in ['peris']:                    
                    ...
                    if epoch < self.opt.warmup_epochs:
                        ...
                        hard_neg_values, hard_neg_indices = neg_score.topk(k=epoch+1, largest=False) #k=epoch+1
                        ...
                        hard_neg_items = all_neg_items[torch.arange(all_neg_items.size(0))[:, None], hard_neg_indices] #neg_items[:pick_num, :] + hard_neg_values.transpose(0,1)
                        all_neg_items = torch.cat([neg_items.transpose(0,1), hard_neg_items], dim=1)
                    else:
                        loss, loss_IS, loss_pref = self.model.compute_loss(batch_data)
                        #not_loss, neg_score = self.model.compute_warmup_loss(batch_data)
                        #hard_neg_values, hard_neg_indices = neg_score.topk(k=5, largest=False)
                        #users, items, times, user_timebins, history, pack_times, all_neg_items, item_timebins, item_labels, neighbors, neighbors_time, neighbors_freqbin = batch_data                                                       
    ...
    def run(self, repeats):
        results = []
        rndseed = [123487, 987324, 327981, 89324, 198349] # random seeds
