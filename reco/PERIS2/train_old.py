...
class Instructor:
    ...
    def train(self):        
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i, batch_data in enumerate(self.trn_loader):
                ...
                if opt.model_name in ['peris']:                    
                    ...
                    if epoch < self.opt.warmup_epochs:
                        loss, neg_score = self.model.compute_warmup_loss(batch_data)       
                        hard_neg_values, hard_neg_indices = neg_score.topk(k=1, largest=False) #k=epoch+1
                        ...
                        #pick_num = self.opt.numneg - (epoch+1)
                        #neg_items = np.random.randint(self.opt.numitem, size = (pick_num, len(users)))
                        #neg_items = torch.LongTensor(neg_items).cuda()
                        #print(neg_items.size(), hard_neg_values.size(), neg_items, hard_neg_values)
                        all_neg_items = all_neg_items[torch.arange(all_neg_items.size(0))[:, None], hard_neg_indices] #neg_items[:pick_num, :] + hard_neg_values.transpose(0,1)
                        #print(i, ': ', all_neg_items.size(), "finish")
                    else:
                        loss, loss_IS, loss_pref = self.model.compute_loss(batch_data)
                        not_loss, neg_score = self.model.compute_warmup_loss(batch_data)
                        hard_neg_values, hard_neg_indices = neg_score.topk(k=1, largest=False)
                        users, items, times, user_timebins, history, pack_times, all_neg_items, item_timebins, item_labels, neighbors, neighbors_time, neighbors_freqbin = batch_data                                                       
                        all_neg_items = all_neg_items[torch.arange(all_neg_items.size(0))[:, None], hard_neg_indices]
            ...
            if opt.model_name in ['peris']:            
                print(('(%.1fs, %.1fs)\tEpoch [%d/%d], TRN_ERR : %.4f, TRN_IS_ERR : %.4f, TST_ERR : %.4f, v_score : %5.4f, tHR@10 : %5.4f'% (elapsed, evalt, epoch, self.opt.num_epoch, batch_loss/len(self.trn_loader), batch_loss_aux/len(self.trn_loader), batch_loss/len(self.tst_loader), (topHits[10] + topNdcgs[10])/2,  test_topHits[10])))                               
            else:            
    ...
    def run(self, repeats):
        results = []
        rndseed = [3214798, 19832, 8329, 2349829, 203123] # random seeds
